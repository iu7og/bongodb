...
Common::TGetResult TBackend::Get(const Common::TKey& key) {
    if (!IsReady() && !Prepare())
        return Common::TGetResult(Common::EError::NotAvail);
...
Common::TRemoveResult TBackend::Remove(const Common::TKey& key) {
    if (!IsReady() && !Prepare())
        return Common::TRemoveResult(Common::EError::NotAvail);
...
Common::TTruncateResult TBackend::Truncate() {
    if (!IsReady() && !Prepare())
        return Common::TTruncateResult(Common::EError::NotAvail);
    return Processor->Truncate();
}

Common::TPutResult TBackend::Put(Common::TKey&& key, Common::TValue&& value) {
    if (!IsReady() && !Prepare())
        return Common::TPutResult(Common::EError::NotAvail);
    return IsForCurrentShard(key)
        ? Processor->Put(std::move(key), std::move(value))
        : ChooseReplica(key)->Client->Put(std::move(key), std::move(value));
}

bool TBackend::IsForCurrentShard(const Common::TKey& key) {
    return Processor->GetShardKey() == Shards.ShardFn(key);
}
