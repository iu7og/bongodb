...
THttpRequest THttpRequest::TruncateRequest() { return THttpRequest(EOperationType::Truncate); }
...
THttpRequest THttpRequest::StreamRequest(const Common::IStreamCommand& streamCommand, const Common::TVersion& version) {
    ...
    return THttpRequest(EOperationType::Stream, std::move(key), std::move(value), std::move(version),
...
THttpRequest THttpRequest::FromPocoHttpRequest(const Poco::Net::HTTPRequest& request, const THttpRequest::TBody& body) {
    ...
    auto value =
        jsonObject->has("value") ? std::optional{jsonObject->get("value").convert<Common::TValue>()} : std::nullopt;
    auto version = jsonObject->has("version") ? std::optional{jsonObject->get("version").convert<Common::TVersion>()}
                                              : std::nullopt;
    auto streamCommandType =
...
EOperationType THttpRequest::GetType() { return OperationType; }

Common::TKey THttpRequest::ExtractKey() {
    if (!Key) {
        poco_error(Logger, "ExtractKey, trying to extract null key");
...
Common::TValue THttpRequest::ExtractValue() {
    if (!Value) {
        poco_error(Logger, "ExtractValue, trying to extract null value");
        throw std::runtime_error("trying to extract null value");
    }
    return std::move(Value.value());
}

std::pair<THttpRequest::TPath, THttpRequest::TMethod> THttpRequest::GetPathAndMethod(EOperationType operationType,
                                                                                     const Common::TKey& key) {
    switch (operationType) {
        case EOperationType::Get:
            return {"/" + key, Poco::Net::HTTPRequest::HTTP_GET};
        case EOperationType::Put:
            return {"/", Poco::Net::HTTPRequest::HTTP_POST};
        case EOperationType::Delete:
            return {"/", Poco::Net::HTTPRequest::HTTP_DELETE};
        case EOperationType::Truncate:
            return {"/", Poco::Net::HTTPRequest::HTTP_POST};
        case EOperationType::Stream:
            return {"/stream", Poco::Net::HTTPRequest::HTTP_POST};
        default:
            throw std::runtime_error("wrong operation type");
    }
}
...
THttpResponse::TBody THttpResponse::GetBody() {
    Poco::JSON::Object obj;
    if (Error) obj.set("error", static_cast<int>(Error.value()));
    if (Value) obj.set("value", Value.value());
